...
# TEST
def run_zero_shot_classifier(text: str, label_names: list):
    cnfg = LlamaConfig()
    cnfg.pretrained_model_path = "./stories42M.pt"
    cnfg.option = "pretrain"
    cnfg.num_labels = len(label_names)
    tknz = Tokenizer()
    sample_input_ids = tknz.encode(text, bos=True, eos=True)
    sample_input_ids = torch.tensor([sample_input_ids])

    lzsc = LlamaZeroShotClassifier(config=cnfg, tokenizer=tknz, label_names=label_names)
    predictions = lzsc(sample_input_ids)
    print(predictions)


# run_zero_shot_classifier("apple", ["fruits", "cars", "neither"])
